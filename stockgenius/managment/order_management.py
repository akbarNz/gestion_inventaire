...
def order_management(inventory:Inventory):
    ...
    while not done:
        message = f"{'-'*106}\nTo create an order enter [c], to remove an order enter [r], to search an order enter [s], to list all orders enter [l], to quit order management enter [q]\n{'-'*106}"
        print(message)
        opt = choose_option()
...
def choose_option():
    ...
    """
    opt = input("enter an option [c/r/s/l/q]: ")
    valid = False

    while not valid:
        if len(opt) == 1 and opt.lower() in 'crslq':
            valid = True
        else:
